...
def translation(word):
    ...
    try:
        result_text = translator.translate(word, src='ko', dest='en')
    except:
        return word
...
def img_blur_text(font_path, image, bboxs, texts, mag=30):
    ...
    text_result = sorted(text_result, key=lambda x: (x[1], x[0]), reverse=False)

    threshold = 19  # 줄 간격 역치값

    dy = list()
    pre_y = text_result[0][1]
    for i in text_result:
        dy.append(i[1] - pre_y)
        pre_y = i[1]

    index = detect_peaks(dy)
    indexlist = list()
    for i in index:
        indexlist.append(dy[int(i)])
    print(indexlist)

    index2 = detect_peaks(indexlist)
    peaks = list()
    for i in index2:
        if indexlist[int(i)] > threshold:
            peaks.append(indexlist[int(i)])
    peaks.sort()
    peak = peaks[0]
    ...
    for i in text_result:
        if i[1] - pre_y < peak:
            ...
            sameline = sorted(sameline, key=lambda x: (x[0]), reverse=False)
            text_line.append(sameline)
            sameline = [i, ]
            pre_y = i[1]
    sameline = sorted(sameline, key=lambda x: (x[0]), reverse=False)
    text_line.append(sameline)
